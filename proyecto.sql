...
--CREAR SECUENCIAS
CREATE SEQUENCE SEQ_COMPANNIA INCREMENT BY 1 START WITH 1 MAXVALUE 99999999999999999999;
CREATE SEQUENCE SEQ_TIPO_ARTICULO INCREMENT BY 1 START WITH 1 MAXVALUE 9999999999999999999;
...
COMMIT;
----ENCA_FACT
INSERT INTO ENCA_FACT VALUES('01','01',TO_DATE('4/04/18','DD/MM/YY'),'01','38000.99','A','1');
INSERT INTO ENCA_FACT VALUES('02','02',TO_DATE('4/04/18','DD/MM/YY'),'01','40000.99','A','2');
INSERT INTO ENCA_FACT VALUES('02','03',TO_DATE('4/04/18','DD/MM/YY'),'01','50000.00','A','2');
INSERT INTO ENCA_FACT VALUES('01','04',TO_DATE('4/04/18','DD/MM/YY'),'01','49000.99','A','2');
INSERT INTO ENCA_FACT VALUES('01','05',TO_DATE('4/04/18','DD/MM/YY'),'01','4000.99','A','2');
COMMIT;
----ARTICULOS
...
FROM ARTICULOS, COMPANNIA 
 ...
 AND ESTADO IN ('A','a')
 AND PRECIO_UNIT >= 50000
 GROUP BY DESCRIP_CIA;
--2.
DELETE FROM ENCA_FACT EF
WHERE EF.ESTADO IN ('I','i')
AND EF.CLIENTE = (
    SELECT CEDULA FROM CLIENTE CL, COMPANNIA CO
    WHERE CL.NOMBRE = 'ACME' AND
    CL.PROVINCIA = 'SJ' AND
    CL.ESTADO IN ('A','a') AND
    CL.FECHA_INGRESO > TO_DATE('01/09/08','DD/MM/YY') AND
    CL.FECHA_INGRESO < TO_DATE('31/03/09','DD/MM/YY') AND
    CL.NO_CIA=CO.NO_CIA AND
    DESCRIP_CIA = 'OFICINA CENTRAL')
AND EF.MONTO_FACT < 50000;


--Parte IV
----PROCEDIMIENTO
CREATE OR REPLACE PROCEDURE HIST_CONTADO AS
BEGIN
SELECT EXTRACT(YEAR) FROM FECHA_FACT, EXTRACT(MONTH) FROM FECHA_FACT, COUNT(NUM_FACT),SUM(MONTO_FACT)
FROM ENCA_FACT
INTO HIST_CONT;

----CURSOR
CURSOR CU_CONTADO(
ANNO NUMBER(4) := EXTRACT(YEAR FROM sysdate),
MES NUMBER(2) := EXTRACT(MONTH FROM sysdate))
IS SELECT * FROM ENCA_FACT WHERE FORMA_PAGO='1';
OPEN CU_CONTADO;
...
CREATE OR REPLACE TRIGGER TRIGGER1
 ...
 BEGIN
 SELECT ESTADO INTO EST FROM CLIENTE WHERE CEDULA = CL;
 IF EST = 'i' OR EST = 'I'
 THEN
...
CREATE OR REPLACE TRIGGER TRIGGER2
 BEFORE INSERT ON DETA_FACT
 FOR EACH ROW
DECLARE
 STCK ARTICULOS.STOCK_BODEGA%TYPE;
 NUEVO_COD DETA_FACT.COD_ART%TYPE := :NEW.COD_ART;
 NUEVO_CANT DETA_FACT.CANTIDAD%TYPE := :NEW.CANTIDAD;
BEGIN
 SELECT STOCK_BODEGA INTO STCK FROM ARTICULOS WHERE COD_ART = NUEVO_COD;
 IF STCK < NUEVO_CANT
 THEN 
  RAISE_APPLICATION_ERROR (-20000, 'La cantidad es mayor que el stock en bodega');
 END IF;
 END;
