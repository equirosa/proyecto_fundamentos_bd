...
INSERT INTO TIPO_ARTICULO VALUES(SEQ_TIPO_ARTICULO.NEXTVAL,'TRONILLOS');

--CUMPLIMIENTO CON PARTE III
--1.
--2.
DELETE FROM ENCA_FACT AS EF (
WHERE EF.ESTADO IN ('I','i') AND (SELECT CL.NOMBRE FROM CLIENTE AS CL WHERE CL.CEDULA=EF.CLIENTE AND CL.NOMBRE='ACME' AND CL.PROVINCIA='SAN JOSE' AND CL.ESTADO IN ('A','a') AND (TO_DATE('01/09/08','DD/MM/YY') < CL.FECHA_INGRESO < TO_DATE('31/03/09','DD/MM/YY')) AND SELECT DESCRIP_CIA FROM COMPANNIA AS COMP WHERE COMP.NO_CIA=CL.NO_CIA AND COMP.DESCRIP_CIA='OFICINA CENTRAL') AND (SELECT PRECIO_UNIT FROM DETA_FACT AS DET WHERE DET.NUM_FACT=EF.NUM_FACT AND EF.PRECIO_UNIT<=50000));
